...
Your OAS 3 API might want to define additional service information, and that's fine - you're welcome to add top-level as well as nested fields. To do that, use the `allOf` OAS 3 construct in your schema. Suppose, for example, we want to add a new required string field named `field`:
